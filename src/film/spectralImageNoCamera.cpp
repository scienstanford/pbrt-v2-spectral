...
void SpectralImageNoCameraFilm::AddSample(const CameraSample &sample,
...
void SpectralImageNoCameraFilm::Splat(const CameraSample &sample, const Spectrum &L) {
    if (L.HasNaNs()) {
        Warning("SpectralImageNoCameraFilm ignoring splatted spectrum with NaN values");
        return;
    }
...
void SpectralImageNoCameraFilm::GetSampleExtent(int *xstart, int *xend,
...
void SpectralImageNoCameraFilm::GetPixelExtent(int *xstart, int *xend,
                               int *ystart, int *yend) const {
    *xstart = xPixelStart;
...
//Andy added
void SpectralImageNoCameraFilm::ParseConversionMatrix(string filename){
...
//Andy changed
void SpectralImageNoCameraFilm::WriteImage(float splatScale) {
...
void SpectralImageNoCameraFilm::UpdateDisplay(int x0, int y0, int x1, int y1,
    float splatScale) {
}

//Andy: added conversionmatrix file here
SpectralImageNoCameraFilm *CreateSpectralImageNoCameraFilm(const ParamSet &params, Filter *filter) {
    ...
    SpectralImageNoCameraFilm * newFilm = new SpectralImageNoCameraFilm(xres, yres, filter, crop, filename, openwin);
